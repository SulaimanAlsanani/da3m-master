...
const Language = ({ className }: { className?: string }) => {
  ...
  const languageText = t("languageText");
  ...
  return (
    <div className={`flex justify-center h-[42px] w-[42px] ${className}`}>
      <button className="flex flex-col items-center" onClick={toggleLanguage}>
        {lang === "ar" ? (
              <svg
        ...
        <span className="text-[#939393] text-[8px]">{languageText}</span>
      </button>
    </div>
  );
};
