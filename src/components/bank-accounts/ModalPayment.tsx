...
if(paymentMethod === "online"){
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-[#9F7A32]/[0.31] z-[9999] p-4 ">
      <div className="bg-white mt-6 pt-[58px] px-[45px] pb-[26px] rounded-[29px] w-full max-w-[721px] max-h-[486px] h-full flex flex-col justify-center items-center relative">
        {/* زر الإغلاق */}
        <div
        ...
        <div className="flex flex-col items-center w-full">
          ...
          {/* buttons */}
          <div className="grid grid-cols-3 items-center justify-center w-full gap-[10px]">
            <button
...
اونلاين            </button>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="w-full">
            ...
            <div className="flex">
              <p className="py-10 text-gray-500">  سيتم نحويلك الي بوابه الدفع الالكتروني بعد تاكيد الدفع</p>
            </div>
            </>}

            {/* زر الإرسال */}
            <button
            disabled={Math.round(data?.data?.balance)=== 0}
              className={`mt-[20px] bg-[#9F7A32] w-full  h-[54px] text-white px-6 py-2 rounded-[14px] text-[14px] ${Math.round(data?.data?.balance)=== 0 &&online ? "cursor-not-allowed opacity-50": ""}`}
              //   onClick={onClose}
            >
              تأكيد الدفع
            </button>
